...
STAGE = 1 #@param [0, 1, 2, 3, 4, 5, 6] {type:"raw"}
CLUSTER_ALGORITHM = "DBSCAN" #@param ["KMeans", "AffProp", "DBSCAN"]

AVG_EXPECTED_QUESTIONS_PER_CLUSTER = 50
AVG_EXPECTED_SENTENCES_PER_CLUSTER = 5
...
def step_load_data():
    if STAGE < 1:
        df = pd.read_csv(default_path + 'data/meta.csv')
        df['text'] = ""
    else:
        df = pd.read_csv(default_path + 'data/meta_with_texts.csv')
...
def get_embedings(embed_module, strings, verbose=0):
    embeddings = np.array([])
        ...
        for i in range(num_steps):
            if i * BATCH_SIZE < len(strings):
                embeddings_batch = session.run(
                    similarity_message_encodings, feed_dict={ similarity_input_placeholder: strings[i * BATCH_SIZE : (i + 1) * BATCH_SIZE] }
                )
...
def clusterize_it(embed_module, strings, algorithm, num_clusters, verbose=0):

    embeddings = get_embedings(embed_module, strings, verbose=0)
...
def step_clusterize_questions(embed_module):
    num_clusters = int(len(questions) / AVG_EXPECTED_QUESTIONS_PER_CLUSTER)
    ...
    if STAGE < 2:
        ...
        questions_clusters, center_indexes = clusterize_it(embed_module,
                                                           unique_questions,
                                                           CLUSTER_ALGORITHM,
                                                           num_clusters,
                                                           verbose=1)
        ...
        df.to_csv(default_path + 'data/clustered_questions_df.csv')

    else:
        df = pd.read_csv(default_path + 'data/clustered_questions_df.csv')
    ...
    with tf.Session(graph=tf.Graph()) as sess:
        ...
        for step in range(start_step, num_steps):
            ...
            if step % 100 == 0:
                ...
                save_df['text'] = texts
                save_df.to_csv(default_path + 'data/gpt_questions_df.csv')
...
def step_generate_questions_texts(clustered_questions_df, START_STEP):
    ...
    if START_STEP == 0:
        ...
        df = pd.read_csv(default_path + 'data/gpt_questions_df.csv')

    if START_STEP == len(question_clusters_ids):
        texts = interact_model(typical_questions, df, START_STEP,
                               model_name='345M', seed=77, top_k=40, verbose=1)

    return df


def step_merge_texts_to_questions(gpt_questions_df, clustered_questions_df):
...
def save_articles():
    clustered_questions_df.to_csv(default_path + 'data/articles_by_question.csv')